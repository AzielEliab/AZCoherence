...
def test_cite_json_names_peers_and_door() -> None:
    ...
    assert CITE["slug"] == "azcoherence"
    assert CITE["placement"] == "scoring-review"
    assert CITE["door"] == "fraggate"
    peers = CITE["peers"]
    assert peers["azclce"]["merge"] is False
    assert peers["akm_triad"]["software_tab"] is False
    assert peers["azinterface"]["slug"] == "azinterface"
