...
/**
 * AZCoherence download tracker (Cloudflare Worker).
 *
...
const PROJECT = "azcoherence";
const KEYS = isolatedKeys(PROJECT);

const DEFAULT_ASSET = "azcoherence-0.1.0.tar.gz";
const DEFAULT_OWNER = "AzielEliab";
const DEFAULT_REPO = "AZCoherence";
const DEFAULT_BRANCH = "main";
const GITHUB_RELEASES = "https://github.com/AzielEliab/AZCoherence/releases";
...
async function githubStats(env) {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 AZCoherence-download-tracker", Accept: "application/vnd.github+json" };
  let stars = 0;
  let forks = 0;
  let watchers = 0;
  let release_download_count = 0;
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/AZCoherence", { headers });
    if (repoRes.ok) {
    ...
    const relRes = await fetch("https://api.github.com/repos/AzielEliab/AZCoherence/releases/latest", { headers });
    if (relRes.ok) {
...
# AZCoherence one-click install. Counted download via this Worker.
set -euo pipefail
HOST="${HOST}"
ASSET="${DEFAULT_ASSET}"
...
async function indexHtml(env) {
  return renderHome(await collectStats(env));
}
